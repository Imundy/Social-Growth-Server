...
router.post('/accounts/:accountId/pages', async (req, res) => {
  const { error } = await socialService.addFacebookPage({ accountId: req.params.accountId, pageId: req.query.pageId });
  if (error) {
...
router.get('/accounts/:accountId/pages', async (req, res) => {
  const { error, success } = await socialService.getFacebookPages(req.params.accountId );
  if (error) {
    return res.status(400).json(error);
  }
